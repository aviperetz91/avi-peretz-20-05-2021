...
const CurrentWeather = (props) => {
    ...
    const isFavorite = favorites.some(
        (fav) =>
            selectedLocation.length > 0 && fav.id === selectedLocation[0].Key
    );
    ...
    const renderHeartIcon = () => {
        let heartIcon = (
            <HeartOutlineIcon
                style={{ fontSize: HEART_ICON_SIZE, color: HEART_ICON_COLOR }}
            />
        );
        if (isFavorite) {
            heartIcon = (
        ...
        <div style={getStyles()}>
            ...
            <div className='row align-items-center'>
                ...
                <div className='d-flex align-items-center col-12 col-lg-6 justify-content-center justify-content-lg-end '>
                    ...
                    <div className='p-2'>{renderTemperature()}</div>
                </div>
            </div>
            <div>
                <div className='display-4'>{currentWeather[0].WeatherText}</div>
            </div>
        </div>
    );
};
